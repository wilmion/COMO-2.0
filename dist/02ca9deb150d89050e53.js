...
;// CONCATENATED MODULE: ./src/components/checkboxes.js
var AddEventListenerClickAccions = function AddEventListenerClickAccions() {
  var Inactive_element = document.querySelectorAll('.carousel_back__items__item');
  Inactive_element[0].style.opacity = "1";
  Inactive_element[3].style.opacity = "0.5";
  var width = window.innerWidth; //Carousel Up

  var content = document.querySelector('.carousel_up__items');
  content.scrollLeft = 890;

  if (width < 600) {
    var diferenceWidths = 590 - width;
    content.scrollLeft = 1103 + 0.5 * diferenceWidths;
  } else {
    var _diferenceWidths = 2000 - width;

    content.scrollLeft = 579 + 0.498 * _diferenceWidths;
  }
  ...
  document.addEventListener('click', function () {
    ...
    if (object.innerText == "«") {
      ...
      if (parent.className == "carousel_back__controls") {
        ...
        if (width < 600) {
          _newScrollLeft = content.scrollLeft - 558;
        } else {
      ...
      if (_parent.className == "carousel_back__controls") {
        var _Element = document.querySelector('.carousel_back__items');

        var _newScrollLeft2 = _Element.scrollLeft;

        if (width < 600 && iteration != 3) {
          iteration++;
          _newScrollLeft2 = _Element.scrollLeft + 400;
        } else if (width > 600) {
          _newScrollLeft2 = 1910;
        }
        ...
        _Element.scrollTo({
          top: 0,
          left: _newScrollLeft2,
          behavior: 'smooth'
        });
      } else {
        var _newScrollLeft3 = 0;

        if (width < 600) {
          _newScrollLeft3 = content.scrollLeft + 558;
        } else {
...
var CarouselBackInit = function CarouselBackInit() {
  var width = window.innerWidth;
  var Element = document.querySelector('.carousel_back__items');

  if (width < 1100) {
    var diferense_Scroll = 1100 - width;
    var newScroll = 42 + 0.507 * diferense_Scroll;
